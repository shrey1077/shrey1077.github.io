 ...
 *  change: `clients.ts` and the logofolio manifest keep pointing at the files
 *  they always did, and the board swaps in the trimmed art and the matched
 *  scale in one place. Generated by scripts/prepare_logo_marks.py; an empty
 *  result simply means the board falls back to the untrimmed files. */
...
export function readMarkPlates(): Record<string, MarkPlate> {
  const file = path.join(process.cwd(), "public", "content", "marks", "_marks.json");
  try {
